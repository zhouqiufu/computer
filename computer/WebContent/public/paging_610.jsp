...
<body id="nv_portal" class="pg_index">
	...
	<div id="hd">
		<div class="wp">
			...
			<div id="nv">
				您当前所在的位置：<font color="#FF0000">&nbsp;&nbsp;&nbsp;&nbsp;计协通知浏览&nbsp;&nbsp;&nbsp;&nbsp;</font>
			</div>
...
#portal_block_249 {
	...
	<%
		...
		int maxRowCount = announceService.getTotalResult();// 一共有多少行
		...
		if(firstResult_arg==null||firstResult_arg==""){
			...
			List<AnnounceDomain>  list = announceService.getPagingResult(Integer.parseInt(firstResult_arg), rowsPerPage);
			
			Iterator iterator = list.iterator();
			AnnounceDomain announceDomain = null;
			while(iterator.hasNext()){
				announceDomain = (AnnounceDomain)iterator.next();
		%>
		
		
		<br/>
		<a target="_blank"  href="<%=basePath %><%= announceDomain.getHtml_path()%>"><%= announceDomain.getAnnounceTitle()%></a>	
	  ...
	  <li class="nv_page"><a class='btn' href="<%=basePath%>public/paging_610.jsp?firstResult_arg=0&pageNum_=<%=i %>">
		...
		%>
		<li class="nv_page"><a class='btn' href="<%= basePath%>public/paging_610.jsp?firstResult_arg=<%=temp_pageNum %>&pageNum_=<%=i %>"> 
